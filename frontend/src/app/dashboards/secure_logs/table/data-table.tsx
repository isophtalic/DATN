...
interface DataTableProps<SecureLogsViewer, TValue> {
    columns: ColumnDef<SecureLogsViewer, TValue>[]
    data: SecureLogsViewer[],
    pageCount: number;
    pagination: PaginationState,
    onSetPagination: (pagination: PaginationState) => void
}

export function DataTable<SecureLogsViewer, TValue>({
    columns,
    data,
    pagination,
    pageCount,
    onSetPagination,
}: DataTableProps<SecureLogsViewer, TValue>) {
    ...
    const getStyleActionsCell = (cell: Cell<SecureLogsViewer, unknown>) => {
        return cell.id.includes("actions") ? "w-[50px]" : ""
    }

    const getStyleUpdatedCell = (cell: Cell<SecureLogsViewer, unknown>) => {
    ...
    const viewDetail = (id: string) => {
        router.push(`/dashboards/secure_logs/${id}`)
    }
                        ...
                        {table?.getRowModel().rows?.length ? (
                            table?.getRowModel().rows.map((row, index) => (
                                <TableRow
                                    ...
                                    onClick={() => viewDetail(data[index].sec_id)}
