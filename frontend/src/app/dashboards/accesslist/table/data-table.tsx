...
interface DataTableProps<AccesslistInterface, TValue> {
    columns: ColumnDef<AccesslistInterface, TValue>[]
...
export function DataTable<AccesslistInterface, TValue>({
    columns,
    data,
    pagination,
    pageCount,
    onSetPagination,
}: DataTableProps<AccesslistInterface, TValue>) {
    ...
    const viewDetail = (id: string) => {
        ...
        router.push(`/dashboards/accesslist/${id}`)
    }
    ...
    const getStyleActionsCell = (cell: Cell<AccesslistInterface, unknown>) => {
        return cell.id.includes("actions") ? "w-[50px]" : ""
    }

    const getStyleUpdatedCell = (cell: Cell<AccesslistInterface, unknown>) => {
                    ...
                    <TableBody>
                        {table?.getRowModel().rows?.length ? (
                            table?.getRowModel().rows.map((row, index) => (
                                <TableRow
                                    key={row.id}
                                    onClick={() => viewDetail(data[index].accesslist_id)}
